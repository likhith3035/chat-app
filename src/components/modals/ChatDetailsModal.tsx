...
export default function ChatDetailsModal({ onClose, chatData, onSelectTheme, onUpdateNickname }: ChatDetailsModalProps) {
    ...
    return (
        <div className="fixed inset-0 bg-gray-600/75 dark:bg-black/60 flex items-center justify-center z-50 p-4 animate-fade-in">
            <div className="bg-white dark:bg-slate-900 border border-transparent dark:border-white/10 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col max-h-[80vh]">
                ...
                <div className="flex border-b border-gray-100 dark:border-slate-800">
                    <button
                        className={`flex-1 py-3 text-sm font-semibold transition-colors ${activeTab === 'theme' ? 'text-blue-500 border-b-2 border-blue-500' : 'text-gray-500 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-800/50'}`}
                        onClick={() => setActiveTab('theme')}
                    >
                        Theme
                    </button>
                    <button
                        className={`flex-1 py-3 text-sm font-semibold transition-colors ${activeTab === 'members' ? 'text-blue-500 border-b-2 border-blue-500' : 'text-gray-500 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-800/50'}`}
